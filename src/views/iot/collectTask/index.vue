  ...
  <UView name="数据采集任务">
    ...
    <UViewModal title="数据采集任务管理" :span="[7, 14]" v-model="editModel" :rules="rules" @edit="onEdit">
      <template #default="{model}">
        ...
        <!--        <UTreeSelectItem field="typeId" label="设备类型" labelField="name"-->
        <!--             url="/iot/deviceType/tree" valueField="id" />-->
        <!--        <USelectItem field="modelId" label="设备型号" labelField="model" valueField="id"-->
        <!--                 url="/iot/deviceModel/listByType?typeId={typeId}" />-->
        <!--        <USelectItem field="uid" label="采集设备" labelField="name" valueField="uid"-->
        <!--                 url="/iot/device/listByModelId?modelId={modelId}"  />-->
    ...
    <UDrawer uid="config" v-model="detailModel" title="采集任务配置" placement="top" height="100%" @open="onOpenDetail">
      <ULinkView uid="configView">
        <UViewSearch v-model="detailSearchModel">
          <URow col="search">
            <UInputItem field="deviceSn" label="设备编号" :allowClear="true" />
            <UInputItem field="deviceName" label="设备名称" :allowClear="true" />
        ...
        <UViewModal title="任务详情管理" width="820px" :span="[7, 14]" v-model="detailEditModel" :rules="detailRules"
          @edit="onDetail">
          <template #default="{model}">
            <URow col="modal">
              <UTreeSelectItem field="deviceTypeId" label="设备类型" url="/iot/deviceType/tree" labelField="name"
                valueField="id" />
              ...
              <USelectItem field="childSn" label="子设备" labelField="name" valueField="childSn"
                url="/iot/deviceChild/listByUid?uid={uid}" />
              <USelectItem field="pointGroupId" label="点位组" labelField="name" valueField="id"
                url="/iot/pointGroup/listByModelId?modelId={modelId}" />
              <USelectItem field="collectAction" label="采集动作" url="/iot/collectTask/collectActions" />
              <USelectItem field="storeAction" label="存储动作" url="/iot/collectTask/storeActions" />
              <UTextareaItem field="extend" :span="22" :labelCol="{span: 4}" :wrapperCol="{span: 20}" label="存储配置"
                defaultValue="{}" :rows="7" />
            </URow>
          </template>
    ...
    <UViewTable :columns="columns" :scroll="{x: 800}">
      <template #status="{record, value, text}">
        <span>
          ...
          <ABadge v-else status="orange" />
          {{value}}
        </span>
      </template>
      <template #action="{record}">
        <UFuncTag func="edit" :data="record" url="/iot/collectTask/edit" v-auth="'iot:collectTask:edit'">修改</UFuncTag>
        <UFuncTag func="open" uid="config" :data="record" color="#87d068" v-auth="'iot:collectTask:config'">配置
        </UFuncTag>
        <UFuncTag func="del" :data="record" url="/iot/collectTask/del" v-auth="'iot:collectTask:del'">删除</UFuncTag>
        <UFuncTag func="ajax:confirm" url="/iot/collectTask/status" :data="record" :params="getStatusParams"
          :config="{confirmTitle: '采集任务切换', confirmContent: `确认切换到[${record.status=='run' ? '停止' : '运行'}]?`}">
          {{record.status=='run' ? '停止' : '运行'}}
        </UFuncTag>
        <UFuncTag func="link" :url="'/iot/collectData?taskId='+record.id" :data="record"
          v-auth="'iot:collectTask:data'">数据</UFuncTag>
      </template>
    </UViewTable>
  </UView>
</template>
  ...
  export default {
    ...
    setup () {
      ...
      let detailColumns = ref([
        { field: "deviceName", title: "采集设备", width: 180, ellipsis: true },
        { field: "deviceSn", title: "设备编号", width: 180, ellipsis: true },
        { field: "childName", title: "子设备", width: 160, ellipsis: true },
        { field: "childSn", title: "子设备编号", width: 160, ellipsis: true },
        { field: "pointGroupName", title: "点位组", width: 160, ellipsis: true },
        { field: "collectAction", title: "采集动作", width: 200, url: "/iot/collectTask/collectActions", ellipsis: true },
        { field: "storeAction", title: "存储动作", width: 200, url: "/iot/collectTask/storeActions", ellipsis: true },
