  ...
  <UView name="产品类型">
      ...
      <template #right>
        <UViewSearch v-model="searchModel">
          <URow col="search">
            ...
            <UButton func="query" url="/iot/productType/view" ref="searchRef">搜索</UButton>
    ...
    <UViewModal title="产品类型管理" :span="[7, 14]" v-model="editModel" :rules="rules" @edit="editOpen">
      <template #default="{model}">
        ...
        <UTreeSelectItem field="pid" label="父类型" url="/iot/productType/tree" labelField="name" valueField="id"/>
        <UInputItem field="alias" label="类型别名" />
...
export default {
  ...
  methods: {
    editOpen(editModel) {
      if(!editModel['pid']) {
        editModel['pid'] = this.searchModel['pid']
      }
    },
